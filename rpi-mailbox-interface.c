...
/* Add a property tag to the current tag list. Data can be included. All data is uint32_t
 * @param tag
 */
void RPI_PropertyAddTag(rpi_mailbox_tag_t tag, ... ){
	...
	switch(tag){
		...
		case TAG_GET_CLOCK_RATE:
			pt[pt_index++] = 8;
			pt[pt_index++] = 0;
			pt[pt_index++] = va_arg(v1, int);
			pt[pt_index++] = 0;
			break;
		case TAG_SET_CLOCK_RATE:
			pt[pt_index++] = 12;
			pt[pt_index++] = 0; /* Request*/
			pt[pt_index++] = va_arg(v1, int); /* Clock ID */
			pt[pt_index++] = va_arg(v1, int); /* Rate (in Hz)*/
			pt[pt_index++] = va_arg(v1, int); /* Skip the turbo setting if == 1 */
			break;
		...
		case TAG_SET_VIRTUAL_OFFSET:
			...
			if( ( tag == TAG_SET_PHYSICAL_SIZE) || (tag == TAG_SET_VIRTUAL_SIZE) || (tag == TAG_SET_VIRTUAL_OFFSET) || (tag == TAG_TEST_PHYSICAL_SIZE) || (tag == TAG_TEST_VIRTUAL_SIZE)){
				pt[pt_index++] = va_arg(v1, int); /* Width*/
				pt[pt_index++] = va_arg(v1, int); /* Height */
			}else{
				pt_index += 2;
			}break;
		...
		case TAG_SET_OVERSCAN:
			...
			if( (tag == TAG_SET_OVERSCAN)){
				pt[pt_index++] = va_arg(v1, int); /*Top pixels */
				pt[pt_index++] = va_arg(v1, int); /* Bottom pixels */
				pt[pt_index++] = va_arg(v1, int); /* Left pixels*/
				pt[pt_index++] = va_arg(v1, int); /* Right pixels*/
			}else{
				pt_index += 4;
			}break;
		default:
